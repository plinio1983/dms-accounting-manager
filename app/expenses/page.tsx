    ...
    return <div className="expense-column-panel" aria-label="Grafico a colonne per categorie di spesa">
        ...
        {groupedData.length ? <div className="expense-column-chart">
            ...
            <span className="expense-column-bar" style={{height: `${height}%`}}>
              {item.residual > 0 ?
                  <span className="expense-column-residual" style={{height: `${residualHeight}%`}}/> : null}
            </span>
                    </div>
...
export default async function ExpensesPage({searchParams}: {
    ...
    const mobileSortedExpenses = [...filteredExpenses].sort((a, b) => {
        ...
        switch (mobileSort) {
            case 'receivedDate_asc':
                return compareDate(a.receivedDate, b.receivedDate, 'asc');
            case 'dueDate_desc':
                return compareDate(a.dueDate, b.dueDate, 'desc');
            case 'dueDate_asc':
                return compareDate(a.dueDate, b.dueDate, 'asc');
            case 'paymentDate_desc':
                return compareDate(a.paymentDate, b.paymentDate, 'desc');
            case 'paymentDate_asc':
                return compareDate(a.paymentDate, b.paymentDate, 'asc');
            ...
            case 'category_asc':
                return compareText(a.category?.name, b.category?.name, 'asc');
            case 'category_desc':
                return compareText(a.category?.name, b.category?.name, 'desc');
            case 'bank_asc':
                return compareText(a.payments[0]?.bank?.name, b.payments[0]?.bank?.name, 'asc');
            case 'channel_asc':
                return compareText(a.payments[0]?.paymentMethod.name, b.payments[0]?.paymentMethod.name, 'asc');
            case 'amount_desc':
                return compareNumber(a.amount, b.amount, 'desc');
            case 'amount_asc':
                return compareNumber(a.amount, b.amount, 'asc');
            case 'paidAmount_desc':
                return compareNumber(a.paidAmount, b.paidAmount, 'desc');
            case 'paidAmount_asc':
                return compareNumber(a.paidAmount, b.paidAmount, 'asc');
            case 'residual_desc':
                return compareNumber(residualA, residualB, 'desc');
            case 'residual_asc':
                return compareNumber(residualA, residualB, 'asc');
            case 'paymentStatus_asc':
                return compareText(a.paymentStatus, b.paymentStatus, 'asc');
            case 'invoiceStatus_asc':
                return compareText(a.invoiceStatus, b.invoiceStatus, 'asc');
            case 'declared_desc':
                return compareNumber(Number(a.isDeclared), Number(b.isDeclared), 'desc');
            case 'electronicInvoice_desc':
                return compareNumber(Number(a.hasElectronicInvoice), Number(b.hasElectronicInvoice), 'desc');
            case 'complete_desc':
            ...
            case 'recurring_desc':
                return compareNumber(Number(a.isRecurring), Number(b.isRecurring), 'desc');
            case 'createdAt_desc':
            ...
            default:
                return compareDate(a.receivedDate, b.receivedDate, 'desc');
        }
    });
    ...
    const activeFilterItems = [
        ...
        paymentStatusFilter && {
            ...
            value: optionLabel(paymentStatusOptions, paymentStatusFilter)
        },
        ...
        electronicInvoiceFilter && {
            ...
            value: electronicInvoiceFilter === 'yes' ? 'Si' : 'No'
        },
            ...
            {activeFilterItems.length ? <div className="recurring-active-filters">
                <div>
                    <div className="flex justify-start align-start">
                        <span className="flex-grow recurring-active-filters-title">Filtri attivi</span><Link className="btn btn-xs btn-neutral recurring-active-filters-reset" href="/expenses"><span className="btn-icon">×</span> Reset</Link>
                    </div>
                    <div className="flex justify-end align-start">
                        <div className="recurring-active-filter-tags">{activeFilterItems.map(item =>
